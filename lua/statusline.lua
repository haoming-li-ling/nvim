...
local set_hl_groups = function()
  ...
  local statusline_groups = {
    ...
    StatusLineLSP = { fg = "#ffffff", bold = true },
...
local file_component = function()
  ...
  if not icon then
    icon, icon_hl = mini_icons.get("file", buf_name)
  end

  -- if not icon then
  --   icon, icon_hl = mini_icons.get_icon_by_filetype(ft, { default = true })
  -- end
  ...
  if buftype == "terminal" then
    ...
    elseif display_name:match("^python ?") then
      icon, icon_hl = mini_icons.get_icon_by_filetype("python", { default = true })
    end
  end
...
return {
  ...
  render = function()
    ...
    local components = {
      ...
      (ft == "markdown" or ft == "tex") and wordcount_component() or "",
      git_component(),
      position_component(),
    }
